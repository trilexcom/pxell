...
/**
 * $Id: graphtasks.php,v 1.6 2005/05/24 15:09:08 madbear Exp $
 * 
...
// Add title
$graph->title->Set(' ' . $strings['calendar'] . ' ' . $strings['tasks']);
...
// query for data
if ($viewCalend == 0) {
    $tmpquery = "WHERE (tas.start_date LIKE '" . $dateCalend . "%' OR tas.due_date LIKE '" . $dateCalend . "%') AND tas.assigned_to = '" . $_SESSION['idSession'] . "' ORDER BY tas.due_date";
} else {
    $tmpquery = "WHERE (tas.start_date LIKE '" . $dateCalend . "%' OR tas.due_date LIKE '" . $dateCalend . "%') AND tas.project = '$viewCalend' ORDER BY tas.due_date";
}
...
for ($i = 0; $i < $comptListTasks; $i++) {
    ...
    if ($listTasks->tas_milestone[$i] != '0') {
        ...
        // set fill color to priority level
